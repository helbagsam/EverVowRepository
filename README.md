...
3. **Lynk.id**: masuk ke Settings → Integrations → Webhooks di dashboard Lynk.id kamu, isi URL Webhook dengan `https://<domain-kamu>/api/webhook/lynkid` (path lengkap, bukan cuma domain — endpoint root tidak menerima POST dan akan balas 405), klik Save URL. Merchant Key muncul setelah URL disimpan — isi persis ke `LYNKID_WEBHOOK_SECRET`.
4. Payload & skema autentikasi sudah diverifikasi langsung dari dokumentasi resmi Lynk.id (link "See Webhook Documentations" di halaman setting webhook mereka, sebuah Postman collection): event yang diproses adalah `payment.received`, autentikasi via header `X-Lynk-Signature` berisi SHA256 dari `grandTotal + refId + message_id + merchantKey` (lihat `verifySignature()` di `src/app/api/webhook/lynkid/route.ts`). Tombol **"Test URL"** Lynk.id sendiri mengirim event connectivity-check (`test_event`) tanpa signature sama sekali — endpoint ini sengaja balas 200 untuk event manapun selain `payment.received` supaya tombol itu selalu sukses, tanpa melonggarkan validasi signature untuk event pembayaran sungguhan.
...
Endpoint webhook menolak (401) event `payment.received` kalau `LYNKID_WEBHOOK_SECRET` kosong atau signature-nya tidak cocok — jadi wajib diisi sebelum dipasang di Lynk.id. Endpoint juga idempotent: kalau Lynk.id kirim webhook dobel untuk order yang sama (retry), tidak akan membuat lisensi dobel atau kirim email dobel. Sudah diverifikasi lewat test manual (buat lisensi via webhook → retry webhook sama → cek `/claim` → login pakai email+kode) — lihat riwayat commit untuk detail.
